...
#        img_height = 30
        ...
        if self.tick_image_mode=='trailing':     #images move along with the bar
            if self.orientation=='h':
                ...
                len_bar = length
                xybox_val = (-38,0) if length < 30 else (-38,0) #Kalau di dalam
#                xybox_val = (38,0) #Kalau mau gambarnya di luar bar pakai ini
    ...
    def prepare_axes(self, ax):
        ...
        ax.spines['left'].set_color('#03346E')
        ax.spines['top'].set_linewidth(3)
        ax.spines['top'].set_color('#03346E')
    ...
    def add_bar_labels(self, ax, bar_location, bar_length):
        if self.bar_textposition:
            ...
            delta = .01 if self.bar_textposition == 'outside' else -.008 #delta=Posisi barnya jarak dari bar ke text
